...
## Option 1: Run with the Prebuilt Docker Image

If your platform supports the published Docker image, run:
...
If the prebuilt image does not work on your machine, build the image locally:
...
This is the recommended fallback for macOS Apple Silicon users.
...
Run the main pricing script:

```bash
python main.py
```

Generate the notebook HTML report:

```bash
jupyter nbconvert --to html --execute pricing.ipynb --ExecutePreprocessor.startup_timeout=120 --ExecutePreprocessor.timeout=600
```

If running in a non-interactive environment, use a non-interactive matplotlib backend:

```bash
MPLBACKEND=Agg jupyter nbconvert --to html --execute pricing.ipynb --ExecutePreprocessor.startup_timeout=120 --ExecutePreprocessor.timeout=600
```
...
`pricing.html` contains the executed notebook report. `bs_vs_heston.png` contains the Black-Scholes vs Heston price comparison plot generated by `main.py`.

## Recommended Docker Image Publishing Fix
